...
function Home() {
  ...
  const getUsersInformation = () => {
    read("/api/v1/information")
      .then((res) => setUsersInformation(res))
      .catch(console.error);
  };
  ...
  return (
    <div className="Home">
      ...
      <Button variant="info" onClick={getAllApi}>
        Get all api that open for you
      </Button>
      {apiOptions &&
        [
          ...
          apiOptions.map((user) => (
            <span className="tableContainer" key={user.description + user.name}>
              ...
              <div className="userPassword">{`${JSON.stringify(
                user.example
              )}`}</div>
            </span>
          ))
        )}
      <Button variant="success" onClick={getUsersInformation}>
      ...
      <Button variant="warning" onClick={getAdminInformation}>
        Try to get information from server
      </Button>
      {aadminInformation &&
        [
